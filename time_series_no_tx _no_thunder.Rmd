...
```{r}
states_to_remove <- c("AMERICAN SAMOA", "ATLANTIC NORTH", "ATLANTIC SOUTH", "DISTRICT OF COLUMBIA", "E PACIFIC", "GULF OF ALASKA", "GULF OF MEXICO", "GUAM", "HAWAII WATERS", "LAKE ERIE", "LAKE HURON", "LAKE MICHIGAN", "LAKE ONTARIO", "LAKE ST CLAIR", "LAKE SUPERIOR", "PUERTO RICO", "ST LAWRENCE R", "VIRGIN ISLANDS", "TEXAS", " ")
...
```{r}
storms_to_remove <- c("Thunderstorm Wind")
...
for (year in years) {
  ...
  if (exists(df_name)) {
    assign(df_name, get(df_name)[!get(df_name)$event_type %in% storms_to_remove, ])
  } else {
