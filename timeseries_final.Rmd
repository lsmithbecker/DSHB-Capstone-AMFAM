...
# Reading in each file and changing to snake case
```{r}
...
# I am removing inconsistent states to ensure all dataframes have the same states - specifically to the continental united states. I've assigned all states that need to be removed to a variable, created "years", and am iterating it throuhg a function that applies it to every year/dataframe
```{r}
...
# This function outputs the frequency of states for all years and provides them in a table
```{r}
...
# this function creates a table/DF by taking info from each state_freq_year and renames the columns so there is a wide format of the data frame for states
```{r}
...
# This code provides the frequencyfor each state by year in long format 
```{r}
...
# This chunk is a heat map of state frequency by year 
```{r}
...
# this is a heatmap of state storm and year frequency
```{r}
...
#This is a scatterplot of storm event frequency by state across all years 
```{r}
state_freq_long <- state_freq %>%
...
# Here are 25 scatterplots of storm event frequency in each year. 
```{r}
...
# this is a line chart that shows each state frequenct change between 1996 and 2021. 
```{r}
state_start_end <- state_freq_long %>% dplyr::filter(year %in% c(1996, 2021))
...
#This is showing me the frequency in descending order.
```{r}
state_freq_ordered <- state_freq_long %>%
  group_by(state) %>%
...
#similar to the visual representation, this creates a table of the states in descending order with the highest frequency change between 1996 and 2021 with a solid frequency number 
```{r}
state_freq_change <- state_freq %>%
  mutate(Change = freq_2021 - freq_1996) %>%
  arrange(desc(Change))


state_freq_change_result <- state_freq_change %>%
  dplyr::select(state, freq_1996, freq_2021, Change)


state_freq_change_result

```


# This is the same but is an average change of in states from 1996 to 2021

```{r}
state_freq_avg <- state_freq_long %>%
  group_by(state) %>%
  arrange(state, year) %>% 
  mutate(Yearly_Change = Frequency - dplyr::lag(Frequency)) %>% 
  summarise(Average_Change = mean(Yearly_Change, na.rm = TRUE)) %>% 
...
#These tables helped me identify which events were shared and differed among years and 
```{r}
...
#This is no longer relevant after realizing merging the groups actually hurt our data output 
```{r}
##no longer relevant 
...
#This function creates tables to show the storm frequency for each year
```{r}
...
#Similar to state, this creates a df for event type and frequency for each year 
```{r}
...
#a bar chart showing the frequency of each event by year
```{r}
...
# This chart shows the variation of year on the x-axis and frequency on the Y with each color representing the each event
```{r}
ggplot(storm_freq_long, aes(x = Year, y = Frequency, color = event_type, group = event_type)) +
  ...
  theme(legend.position = "right")
```
heat map of storm event frequency by year 
```{r}
...
#table showing storm event by most frequent to least with raw numbers
```{r}
...
#Storm event frequency change in raw number from 1996 to 2021
```{r}
storm_freq_change <- storm_freq %>%
  mutate(Change = freq_2021 - freq_1996) %>%
  arrange(desc(Change))


storm_freq_change_result <- storm_freq_change %>%
  select(event_type, freq_1996, freq_2021, Change)


storm_freq_change_result

```

#I used these lines to see where the first instances of these events took place
```{r}

hurricane_first_year <- storm_freq_long %>%
  plotly::filter(event_type == "Hurricane") %>%
  summarise(First_Year = min(Year, na.rm = TRUE))

thunderstorm_first_year <- storm_freq_long %>%
  plotly::filter(event_type == "Thunderstorm Wind") %>%
  summarise(First_Year = min(Year, na.rm = TRUE))


hurricane_first_year
thunderstorm_first_year

```
```{r}

hurricane_first_year <- storm_freq_long %>%
  plotly::filter(event_type == "Hurricane" & !is.na(Frequency)) %>%
  summarise(First_Year = min(Year))


thunderstorm_first_year <- storm_freq_long %>%
  plotly::filter(event_type == "Thunderstorm Wind" & !is.na(Frequency)) %>%
  summarise(First_Year = min(Year))
...
#This table shows state frequency and year to use in the following chunk 
```{r}
...
```
#this showed the overall change in states in order - some of this I did more than once so that I had them in more accessible locations
```{r}
state_freq_change_result
```

#This shows the average frequency of change in descending order 
```{r}
storm_freq_avg <- storm_freq_long %>%
  group_by(event_type) %>%
...
#these are 25 choropleths showing the change for each year/representing each year 
```{r}
generate_choropleth_map <- function(data, year, map_data) {
  data_filtered <- data %>%
    plotly::filter(year == year) %>%
...
#creates a two column table with frequency and the year 
```{r}
state_freq_long <- state_freq %>%
  pivot_longer(
...
#shows the statistical analysis in descending order
```{r}
ranked_state <- lm_state %>%
  plotly::filter(term == "Year") %>%
  arrange(desc(estimate))  
ranked_state
```

#Interactive bar chart showing storm frequency over the years 
```{r}
p <- ggplot(state_freq_short, aes(x = Year, y = Frequency, fill = as.factor(Year))) +
  geom_bar(stat = "identity") +
  labs(title = "Storm Event Frequency Over Years", x = "Year", y = "Frequency") +
  theme_minimal() +
...
#Interactive line chart showing state/storm frequency
```{r}
...
#Interactive heat map for state storm and yera 
```{r}
...
#Interactive map of storm and state frequency
```{r}
...
#interactive choropleth showing progression/change of storm frequency by state over the 25 year period 
```{r}
us_states <- map_data("state")
...
if (any(is.na(map_data_combined$locations))) {
  ...
  map_data_combined <- map_data_combined %>% plotly::filter(!is.na(locations))
}
...
#mixed effects model regressing frequency on year while using event type as a random effect
```{r}
...
#another lmer model but this time using event type as a fixed and random effect
```{r}
lmer_storm2 <- lmer(Frequency ~ Year + event_type + (1 | event_type), data = storm_freq_long)


summary(lmer_storm2)
```

lmer model regressing frequency on year with state as a random effect
```{r}
lmer_state <- lmer(Frequency ~ Year + (1 | state), data = state_freq_long)
...
lmer model regressing frequency on year with state as a fixed and random effect
```{r}
lmer_state2 <- lmer(Frequency ~ Year + state + (1 | state), data = state_freq_long)
summary(lmer_state2)
```
